...
interface TBInterface (input bit clk, input bit rst_n);
	...
	bit signed [12 : 0] lm_gain;
	bit signed [12 : 0] lm_gain_comp;
	bit in_enable;
...
module LuminanceTransform_TB();
	...
	CLOCK CLOCK1 (clk);
	True2Comp #(3, 13) TCRGBPipline(RGBPipline.lm_gain, RGBPipline.lm_gain_comp);
	True2Comp #(3, 13) TCRGBReqAck(RGBReqAck.lm_gain, RGBReqAck.lm_gain_comp);
	True2Comp #(1, 13) TCGrayPipline(GrayPipline.lm_gain, GrayPipline.lm_gain_comp);
	True2Comp #(1, 13) TCGrayReqAck(GrayReqAck.lm_gain, GrayReqAck.lm_gain_comp);
	LuminanceTransform #(0, 3, 8) 
		LTRGBPipline(RGBPipline.clk, RGBPipline.rst_n, RGBPipline.lm_gain_comp, 
			RGBPipline.in_enable, RGBPipline.in_data, RGBPipline.out_ready, RGBPipline.out_data);
	LuminanceTransform #(1, 3, 8) 
	...
	LuminanceTransform #(0, 1, 8) 
		LTGrayPipline(GrayPipline.clk, GrayPipline.rst_n, GrayPipline.lm_gain_comp, 
			GrayPipline.in_enable, GrayPipline.in_data, GrayPipline.out_ready, GrayPipline.out_data);
	LuminanceTransform #(1, 1, 8) 
	...
	task init_file();
		...
		$fwrite(fo, "%s\n", imconf);
		if(imconf == "RGB") begin
			fst = $fscanf(fi, "%b", RGBPipline.lm_gain);
			RGBReqAck.lm_gain = RGBPipline.lm_gain;
		end else if(imconf == "L") begin
			fst = $fscanf(fi, "%b", GrayPipline.lm_gain);
			GrayReqAck.lm_gain = GrayPipline.lm_gain;
		end
	endtask : init_file
