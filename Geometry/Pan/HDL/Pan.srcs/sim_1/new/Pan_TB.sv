...
interface TBInterface (input bit clk, input bit rst_n);
	...
	bit signed[im_width_bits : 0] offset_x_true;
	bit signed[im_width_bits : 0] offset_y_true;
	bit signed[im_width_bits : 0] offset_x;
	bit signed[im_width_bits : 0] offset_y;
...
module Pan_TB();
	...
	CLOCK CLOCK1(clk);
	True2Comp #(2, im_width_bits + 1) 
		TCRGBPipeline({RGBPipeline.offset_x_true, RGBPipeline.offset_y_true}, {RGBPipeline.offset_x, RGBPipeline.offset_y});
	True2Comp #(2, im_width_bits + 1) 
		TCRGBReqAck({RGBReqAck.offset_x_true, RGBReqAck.offset_y_true}, {RGBReqAck.offset_x, RGBReqAck.offset_y});
	True2Comp #(2, im_width_bits + 1) 
		TCGrayPipeline({GrayPipeline.offset_x_true, GrayPipeline.offset_y_true}, {GrayPipeline.offset_x, GrayPipeline.offset_y});
	True2Comp #(2, im_width_bits + 1) 
		TCGrayReqAck({GrayReqAck.offset_x_true, GrayReqAck.offset_y_true}, {GrayReqAck.offset_x, GrayReqAck.offset_y});
	Pan #(0, RGBPipeline.data_width, im_width, im_height, im_width_bits)
		PanRGBPipeline(
			RGBPipeline.clk, RGBPipeline.rst_n, RGBPipeline.offset_x, RGBPipeline.offset_y, 
	...
	Pan #(1, RGBReqAck.data_width, im_width, im_height, im_width_bits)
		PanRGBReqAck(
			RGBReqAck.clk, RGBReqAck.rst_n, RGBReqAck.offset_x, RGBReqAck.offset_y, 
	...
	Pan #(0, GrayPipeline.data_width, im_width, im_height, im_width_bits)
		PanGrayPipeline(
			GrayPipeline.clk, GrayPipeline.rst_n, GrayPipeline.offset_x, GrayPipeline.offset_y, 
	...
	Pan #(1, GrayReqAck.data_width, im_width, im_height, im_width_bits)
		PanGrayReqAck(
			GrayReqAck.clk, GrayReqAck.rst_n, GrayReqAck.offset_x, GrayReqAck.offset_y, 
	...
	task init_file();
		...
		$fwrite(fo, "%s\n", imconf);
		if(imconf == "RGB") begin
			fst = $fscanf(fi, "%b", RGBPipeline.offset_x_true);
			RGBReqAck.offset_x_true = RGBPipeline.offset_x_true;
			fst = $fscanf(fi, "%b", RGBPipeline.offset_y_true);
			RGBReqAck.offset_y_true = RGBPipeline.offset_y_true;
		end else if(imconf == "L") begin
			fst = $fscanf(fi, "%b", GrayPipeline.offset_x_true);
			GrayReqAck.offset_x_true = GrayPipeline.offset_x_true;
			fst = $fscanf(fi, "%b", GrayPipeline.offset_y_true);
			GrayReqAck.offset_y_true = GrayPipeline.offset_y_true;
		end
	endtask : init_file
