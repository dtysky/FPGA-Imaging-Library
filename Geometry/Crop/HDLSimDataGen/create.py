...
def name_format(root, name, ex, conf):
	return '%s-%sx%sx%sx%s' % (name, conf['top'], conf['bottom'], conf['left'], conf['right'])

def conf_format(im, conf):
	size = max(im.size) - 1
	tblr = [conf['top'], conf['bottom'], conf['left'], conf['right']]
	res = ''
	for p in tblr:
		tmp = bin(p)[2:]
		for i in xrange(len(bin(size)) - len(bin(p))):
			tmp = '0' + tmp
		res += tmp + '\n'
	return '%s\n%s' % (im.mode, res)
