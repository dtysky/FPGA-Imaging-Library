...
Give the first output after mul_delay + 1 + ram_RL cycles while the input enable.
...
module Scale_TB();
	...
	CLOCK CLOCK1(clk);
	Scale #(0, GrayPipeline.data_width, im_width, im_height, im_width_bits, 3, 6)
		SCLGrayPipeline(
			GrayPipeline.clk, GrayPipeline.rst_n, GrayPipeline.scale_x, GrayPipeline.scale_y, 
	...
	Scale #(1, GrayReqAck.data_width, im_width, im_height, im_width_bits, 3, 6)
		SCLGrayReqAck(
			GrayReqAck.clk, GrayReqAck.rst_n, GrayReqAck.scale_x, GrayReqAck.scale_y, 
	...
	task init_file();
		...
		fst = $fscanf(fi, "%s", imconf);
		$fwrite(fo, "%s\n", imconf);
		fst = $fscanf(fi, "%b", GrayPipeline.scale_x);
		GrayReqAck.scale_x = GrayPipeline.scale_x;
		fst = $fscanf(fi, "%b", GrayPipeline.scale_y);
		GrayReqAck.scale_y = GrayPipeline.scale_y;
	endtask : init_file
	...
	initial begin
		...
		for (int i = 0; i < fsize; i++) begin;
			...
			init_signal();
			for (int j = 0; j < im_width * im_height + 11; j++) begin
				work_pipeline();
			end
			$fclose(fi);
			$fclose(fo);
