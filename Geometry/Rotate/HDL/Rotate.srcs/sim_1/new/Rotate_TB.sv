...
module Rotate_TB();
	...
	CLOCK CLOCK1(clk);
	Rotate #(0, GrayPipeline.data_width, im_width, im_height, im_width_bits, 3, 6)
		RTTGrayPipeline(
			GrayPipeline.clk, GrayPipeline.rst_n, GrayPipeline.angle, 
	...
	Rotate #(1, GrayReqAck.data_width, im_width, im_height, im_width_bits, 3, 6)
		RTTGrayReqAck(
			GrayReqAck.clk, GrayReqAck.rst_n, GrayReqAck.angle, 
	...
	task init_file();
		...
		fst = $fscanf(fi, "%s", imconf);
		$fwrite(fo, "%s\n", imconf);
		fst = $fscanf(fi, "%b", GrayPipeline.angle);
		GrayReqAck.angle = GrayPipeline.angle;
	endtask : init_file
